...
#include "recipeAUX/OxbridgeMT2/interface/Basic_MCT2_332_Calculator.h"
...
namespace Mt2 {
  ...
  double Basic_MCT2_332_Calculator::mct2_332(const LorentzTransverseVector& visA, 
					     const LorentzTransverseVector& visB,
					     const TwoVector& ptmiss, 
					     const double mEachInvisible) {
    const double mSq = mct2_332_Sq(visA, visB, ptmiss, mEachInvisible);
  ...
  double  Basic_MCT2_332_Calculator::mCT2Fcn::operator()(const std::vector<double>& par) const {
    ...
    double ETj1 = sqrt(theMass1*theMass1 +  thePT1x*thePT1x + thePT1y*thePT1y);
    double ETj2 = sqrt(theMass2*theMass2 +  thePT2x*thePT2x + thePT2y*thePT2y);
